...
const tryGetQrc = keyword => {
  const requestObj = httpFetch(`https://www.oiapi.net/api/QQMusicLyric?keyword=${encodeURIComponent(keyword)}&format=qrc&type=json&n=1`, {
    headers: {
...
export default {
  ...
  getQrcByKeyword(name, singer) {
    const firstSinger = singer.split(/[、,，\s&]/)[0] || singer
    const keywords = [
      `${name} ${singer}`,
      `${name} ${firstSinger}`,
      name,
    ]
    const requestObj = { cancelHttp: () => {} }
    requestObj.promise = keywords.reduce((prev, keyword) => {
      return prev.catch(() => tryGetQrc(keyword))
